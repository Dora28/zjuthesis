...
导叶的作用方式如\autoref{fig:dj_modle}所示。
\begin{figure}[htbp]
    \centering
    \includegraphics[scale=1.0]{3导叶作用方式.png}
    \caption{\label{fig:dj_modle}导叶作用方式}
\end{figure}
...
推进泵设计模型如\autoref{fig:dj_modle}所示。
\begin{figure}[htbp]
    \centering
...
单级推进泵试验模型如\autoref{fig:dj_modle}所示。
\begin{figure}[htbp]
    \centering
    \includegraphics[scale=1.4]{3单级试验模型.png}
    \caption{\label{fig:dj_modle}单级推进泵试验模型}
\end{figure}

为了获得推进泵在不同水动力工况下的噪声数据，
试验中通过固定叶轮转速、改变水洞流速的方法测量了进速系数0.3至1.8范围内的20个工况点下的
水动力性能和噪声数据。
...
将水洞试验测得的单级推进泵推力、转矩和效率等数据无量纲化后，得到如图 4 所
示的单级推进泵敞水性能曲线。从图中可以看出：随着进速系数$J$的增大，
推力系数$K_T$和转矩系数$K_Q$逐渐减小，而推进效率\eta $_0$先增大后减小，
并在 J=1.35 处有最大值 62.7\%。与常见的螺旋桨敞水性能
曲线相比，单级推进泵的敞水性能曲线随进速系数增加时变化趋势平缓，具有较高推进
效率的同时也有较宽的高效工作区。
\begin{figure}[htbp]
    \centering
    \includegraphics[scale=0.8]{3单级敞水性能曲线待修改.png}
    \caption{\label{fig:jisuanyuwangge}单级推进泵敞水性能曲线}
\end{figure}

本实验中利用水听器阵列进行螺旋桨辐射噪声的采集，其固定实验装置如图 4-11 所
示。利用水听器的空间位置布置，能够有效的进行螺旋桨在不同距离上的辐射噪声采集，
水听器基本信息如表 4-3 所示。 

\begin{figure}[htbp]
    \centering
    \includegraphics[scale=0.8]{3单级水听器布置.png}
    \caption{\label{fig:jisuanyuwangge}单级推进泵敞水性能曲线}
\end{figure}


\subsection{推进泵噪声特性分析}
推进泵和螺旋桨等推进设备的噪声频率范围的划分目前还没有统一的标准，其中
...
将水洞实验测得的双级吊舱推进器推力、转矩和效率等数据无量纲化后如图4所示。
从图中可以看出：随着进速系数J的增大，推力系数$K_T$和转矩系数$K_Q$逐渐减小，
而推进效率$η_0$先增大后减小，并在J=1.35处有最大值62.7\%。与常见的螺旋桨敞水性能曲线相比，
双级推进泵的敞水性能曲线随进速系数增加时变化趋势平缓，具有较高推进效率的同时也有较宽的高效工作区。
图4对比了数值模拟与水洞实验的结果，可以看出数值模拟得到的敞水特性曲线与实验结果吻合度较好，
证实所采用的数值模拟方法具有较高精度。
图5对比了数值模拟和实验方法得到的静止部件推力系数随进速系数变化的趋势，
可以看出数值模拟不仅能够预报推进器总体的水动力特性，也可以准确的模拟出各个部件的性能。
